...
{  
  ...
  public:
    bool doTest(const Lexer::ITokenCollection* pTc) override
    {
	//	std::cout << "\ndoTest in DetectAccessSpecifier";
  ...
  /*
  class CommentScope : public IRule
  {
  public:
	  bool doTest(const Lexer::ITokenCollection* pTc) override
	  {
 ...
 /* class HandleCommentScope : public IAction
  {
	  Repository* p_Repos;

  public:
	  HandleCommentScope(Repository* pRepos)
	  {
		  p_Repos = pRepos;
	  }
	  void doAction(const Lexer::ITokenCollection* pTc) override
  ...
  */
  ...
  public:
    ...
    void doAction(const Lexer::ITokenCollection* pTc) override
    {
      ...
      {
        //size_t nameIndex = typeIndex + 1;	//There could be multiple spaces
        std::string name = (*pTc)[typeIndex + 1];
        top->type_ = "class";
        top->name_ = name;
        top->package_ = p_Repos->package();
