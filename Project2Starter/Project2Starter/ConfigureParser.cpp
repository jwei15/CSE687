...
Parser* ConfigParseForCodeAnal::Build()
{
  try
  {
	...
	//std::cout << "In ConfigParseForCodeAnal: "<<pRepo<<std::endl;

    // configure to manage scope
    // these must come first - they return true on match
    // so rule checking continues
	/*
	pCommentScope = new CommentScope();
	pHandleCommentScope = new HandleCommentScope(pRepo);
	pCommentScope->addAction(pHandleCommentScope);
	pParser->addRule(pCommentScope);
	*/
