...
namespace Navigator
{
    public class NavigatorServer
    {
        ...
        void initializeDispatcher()
        {
            ...
            // <---- If the server receives request to pop up server files --->
            Func<CommMessage, CommMessage> PopUpFiles = (CommMessage msg) =>
            {
                string abs_path = msg.arguments[0];
                CommMessage reply = new CommMessage(CommMessage.MessageType.reply);
                reply.to = msg.from;
                reply.from = msg.to;
                reply.command = "PopUpRemote";
                reply.arguments.Add(File.ReadAllText(abs_path));
                return reply;
            };
            messageDispatcher["PopUpFiles"] = PopUpFiles;

            // <---- What happens if the server receives a download request -- >
            Func<CommMessage, CommMessage> Download = (CommMessage msg) =>
            {
                CommMessage reply = new CommMessage(CommMessage.MessageType.reply);
                reply.to = msg.from;
                reply.from = msg.to;
                reply.command = "DownloadConfirmed";
                reply.arguments.Add(File.ReadAllText(ServerEnvironment.root + "/" + msg.arguments[0]));
                reply.arguments.Add(msg.arguments[0]);
                //Our reply argument looks like: content, filename
                return reply;
            };
            messageDispatcher["Download"] = Download;

            // <---- What happens if server receives a submission request -->
            Func<CommMessage, CommMessage> Submit = (CommMessage msg) =>
            {
                CommMessage reply = new CommMessage(CommMessage.MessageType.reply);
                reply.to = msg.from;
                reply.from = msg.to;
                reply.command = "SubmitConfirmed";
                if (msg.arguments[0].Contains('\\'))
                {
                    string[] sArray = msg.arguments[0].Split('\\');
                    File.WriteAllText("../../../ServerFiles" + "/" + msg.arguments[1] + "/" +
                        sArray[sArray.Length -1], msg.arguments[2]);
                }
                else
                {
                    File.WriteAllText("../../../ServerFiles" + "/" + msg.arguments[1] + "/" +
                        msg.arguments[0], msg.arguments[2]);
                }
                return reply;
            };
            messageDispatcher["Submit"] = Submit;
